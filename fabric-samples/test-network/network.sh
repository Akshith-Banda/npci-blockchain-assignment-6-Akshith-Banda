...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating jio Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-jio.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating airtel Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-airtel.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/jio/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Jio and Airtel"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.jio.example.com docker_peer0.airtel.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/jio/msp organizations/fabric-ca/jio/tls-cert.pem organizations/fabric-ca/jio/ca-cert.pem organizations/fabric-ca/jio/IssuerPublicKey organizations/fabric-ca/jio/IssuerRevocationPublicKey organizations/fabric-ca/jio/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/airtel/msp organizations/fabric-ca/airtel/tls-cert.pem organizations/fabric-ca/airtel/ca-cert.pem organizations/fabric-ca/airtel/IssuerPublicKey organizations/fabric-ca/airtel/IssuerRevocationPublicKey organizations/fabric-ca/airtel/fabric-ca-server.db'
